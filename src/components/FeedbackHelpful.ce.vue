    ...
    <details>
      ...
      <form @submit="onSubmit" v-if="showForm">
        <fieldset id="feedbackInputs">
          ...
          <fieldset id="feedbackNoFieldset" v-if="getIsHelpful == Answer.NO">
            <legend class="required">
              <span class="field-name">{{ t("form.selectOptions") }}</span><strong class="required"> ({{ t("form.required") }})</strong>:
            </legend>
            <!-- vee-validate parameters only have to be set on one element with the same name -->
            <checkbox-item 
              name="reasons"
              label-for="infoHardToUnderstand"
              value="infoHardToUnderstand"
              :is-reset="isResetReasons"
              @reset-completed="handleResetReasonsCompleted"
        ...
        <div>
          <div class="alert alert-info">
            ...
            <ul>
              <li><small><strong>{{ t('contact.noReply')}}.</strong> {{ t('contact.forEnquiries')}} <a :href="`${websiteBasePath}${t('contact.opcContactLink')}`">{{ t('contact.contactOpc')}}</a>.</small></li>
              <li><small><strong>{{ t('submissionInformation.noPersonalInfo')}},</strong> {{ t('submissionInformation.personalInfoItems')}}.</small></li>
              <li><small><strong>{{ t('submissionInformation.forMoreInfo')}}</strong>, {{ t('submissionInformation.referTo')}} <a :href="`${websiteBasePath}${t('submissionInformation.termsOfUseLink')}`">{{ t('submissionInformation.termsOfUse')}}</a>.</small></li>
            </ul>
          </div>
          <div>
      ...
      <div v-if="submitSuccessful" id="feedbackSuccess">
        ...
        <div>
          ...
          <p>{{t('contact.noReply')}}. {{t('contact.forEnquiries')}} <a :href="`${websiteBasePath}${t('contact.opcContactLink')}`">{{t('contact.contactOpc')}}</a>.</p>
        </div>
        <!-- </feedback-confirmation> -->
